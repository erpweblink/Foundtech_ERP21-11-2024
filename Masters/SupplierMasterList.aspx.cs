...
public partial class Masters_SupplierMasterList : System.Web.UI.Page
{
    ...
    {
        DataTable Dt = Cls_Main.Read_Table("SELECT * FROM [tbl_suppliermaster] WHERE IsDeleted = 0");
    ...
    protected void GVVendor_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("SupplierMaster.aspx?Id=" + objcls.encrypt(e.CommandArgument.ToString()) + "");
        }
        if (e.CommandName == "RowDelete")
        {
            Cls_Main.Conn_Open();
            SqlCommand Cmd = new SqlCommand("UPDATE [tbl_suppliermaster] SET IsDeleted=@IsDeleted,DeletedBy=@DeletedBy,DeletedOn=@DeletedOn WHERE ID=@ID", Cls_Main.Conn);
            ...
            Cmd.ExecuteNonQuery();
            Cls_Main.Conn_Close();
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Supplier Deleted Successfully..!!')", true);
            FillGrid();
        }
    }
