...
public partial class Production_Painting : System.Web.UI.Page
{
    ...
    {
        DataTable Dt = Cls_Main.Read_Table("SELECT * FROM tbl_ProductionDTLS AS PD INNER JOIN tbl_ProductionHDR AS PH ON PH.JobNo=PD.JobNo where PD.Stage='Painting' AND CONVERT(bigint,ISNULL(TotalQTY,0))>=CONVERT(bigint,ISNULL(OutwardQTY,0)) AND PD.Status<2");
        ...
        try
        {
            if (txtoutwardqty.Text != null && txtoutwardqty.Text != "")
            {
                if (Convert.ToDouble(txtinwardqty.Text) + 1 > Convert.ToDouble(txtoutwardqty.Text))
                {
                    ...
                    cmdselect1.Parameters.AddWithValue("@StageNumber", "5");
                    ...
                    DataTable Dt = Cls_Main.Read_Table("SELECT TOP 1 * FROM tbl_ProductionDTLS AS PD where JobNo='" + txtjobno.Text + "'and StageNumber>4 ");
                    if (Dt.Rows.Count > 0)
                    {
                        int StageNumber = Convert.ToInt32(Dt.Rows[0]["StageNumber"].ToString());
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Saved Record Successfully And Send to the Next..!!');window.location='Painting.aspx';", true);
                    FillGrid();
    ...
    protected void btnsendtoback_Click(object sender, EventArgs e)
    {
        try
        {
            if (txtoutwardqty.Text != null && txtoutwardqty.Text != "")
            {
                Cls_Main.Conn_Open();
                SqlCommand Cmd2 = new SqlCommand("UPDATE [tbl_ProductionDTLS] SET  RevertQty= @RevertQty WHERE StageNumber=@StageNumber AND JobNo=@JobNo", Cls_Main.Conn);
                Cmd2.Parameters.AddWithValue("@StageNumber", 3);
                Cmd2.Parameters.AddWithValue("@JobNo", txtjobno.Text);
                ...
                DataTable Dt = Cls_Main.Read_Table("SELECT TOP 1 * FROM tbl_ProductionDTLS AS PD where JobNo='" + txtjobno.Text + "'and StageNumber<4 order by StageNumber desc");
                if (Dt.Rows.Count > 0)
                {
                    int StageNumber = Convert.ToInt32(Dt.Rows[0]["StageNumber"].ToString());
                    Cls_Main.Conn_Open();
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Saved Record Successfully And Send Back..!!');window.location='Painting.aspx';", true);
                FillGrid();
