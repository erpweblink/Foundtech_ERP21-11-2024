...
public partial class Production_Packing : System.Web.UI.Page
{
    ...
    private void FillGrid()
    {
        ...
        DataTable Dt = Cls_Main.Read_Table("SELECT * FROM tbl_ProductionDTLS AS PD INNER JOIN tbl_ProductionHDR AS PH ON PH.JobNo=PD.JobNo where PD.Stage='Packaging' AND CONVERT(bigint,ISNULL(InwardQTY,0))>=CONVERT(bigint,ISNULL(OutwardQTY,0)) AND PD.Status<2");
    ...
    protected void GVPurchase_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "SendtoNext")
        {
            ...
            DataTable Dt = Cls_Main.Read_Table("SELECT TOP 1 * FROM tbl_ProductionDTLS AS PD where JobNo='" + JobNo + "'and StageNumber>5 ");
            if (Dt.Rows.Count > 0)
            {
        ...
        try
        {
            if (txtoutwardqty.Text != null && txtoutwardqty.Text != "")
            {
                if (Convert.ToDouble(txtinwardqty.Text) + 1 > Convert.ToDouble(txtoutwardqty.Text))
                {
                    ...
                    cmdselect1.Parameters.AddWithValue("@StageNumber", "6");
                    ...
                    DataTable Dt = Cls_Main.Read_Table("SELECT TOP 1 * FROM tbl_ProductionDTLS AS PD where JobNo='" + txtjobno.Text + "'and StageNumber>5 ");
                    if (Dt.Rows.Count > 0)
                    {
                        int StageNumber = Convert.ToInt32(Dt.Rows[0]["StageNumber"].ToString());
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Saved Record Successfully And Send to the Next..!!');window.location='Packing.aspx';", true);
                    FillGrid();
    ...
    protected void btnsendtoback_Click(object sender, EventArgs e)
    {
        try
        {
            if (txtoutwardqty.Text != null && txtoutwardqty.Text != "")
            {
                Cls_Main.Conn_Open();
                SqlCommand Cmd2 = new SqlCommand("UPDATE [tbl_ProductionDTLS] SET  RevertQty= @RevertQty WHERE StageNumber=@StageNumber AND JobNo=@JobNo", Cls_Main.Conn);
                Cmd2.Parameters.AddWithValue("@StageNumber", 4);
                Cmd2.Parameters.AddWithValue("@JobNo", txtjobno.Text);
                ...
                DataTable Dt = Cls_Main.Read_Table("SELECT TOP 1 * FROM tbl_ProductionDTLS AS PD where JobNo='" + txtjobno.Text + "'and StageNumber<5 order by StageNumber desc");
                if (Dt.Rows.Count > 0)
                {
                    int StageNumber = Convert.ToInt32(Dt.Rows[0]["StageNumber"].ToString());
                    Cls_Main.Conn_Open();
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Saved Record Successfully And Send Back..!!');window.location='Packing.aspx';", true);
                FillGrid();
